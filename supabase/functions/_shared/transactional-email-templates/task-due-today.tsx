...
const TaskDueTodayEmail = ({
  ...
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>
      Task due today{taskTitle ? `: ${taskTitle}` : ''}
    </Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>You have a task due today</Heading>
        <Text style={text}>
          {recipientName ? `Hi ${recipientName},` : 'Hi,'} this is a reminder
          that the following task is due{dueDate ? ` on ${dueDate}` : ' today'}.
        </Text>
        <Text style={taskTitleStyle}>{taskTitle || 'Untitled task'}</Text>
        {taskDescription ? (
          <Text style={quote}>{taskDescription}</Text>
        ) : null}
        {link ? (
...
export const template = {
  component: TaskDueTodayEmail,
  subject: (data: Record<string, any>) =>
    `Task due today${data?.taskTitle ? `: ${data.taskTitle}` : ''}`,
  displayName: 'Task due today',
  previewData: {
    recipientName: 'Scott',
