...
export function LiveKpiReport() {
  const sumB25 = MONTHLY.reduce((s, r) => s + r.b25, 0);
  const sumB26P = MONTHLY.reduce((s, r) => s + r.b26p, 0);
  const sumYtdB = MONTHLY.reduce((s, r) => s + r.ytdB, 0);
  const sumI25 = MONTHLY.reduce((s, r) => s + r.i25, 0);
  const sumI26P = MONTHLY.reduce((s, r) => s + r.i26p, 0);
  const sumYtdI = MONTHLY.reduce((s, r) => s + r.ytdI, 0);
  ...
  const luxP = LINE_BOOK.reduce((s, r) => s + r.luxP, 0);
  const luxA = LINE_BOOK.reduce((s, r) => s + r.luxA, 0);
  const swP = LINE_BOOK.reduce((s, r) => s + r.swP, 0);
  const swA = LINE_BOOK.reduce((s, r) => s + r.swA, 0);
  const flP = LINE_BOOK.reduce((s, r) => s + r.flP, 0);
  const flA = LINE_BOOK.reduce((s, r) => s + r.flA, 0);
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b">
                ...
                <th colSpan={4} className="text-center p-2 font-semibold border-l bg-muted/30">Bookings</th>
                <th colSpan={4} className="text-center p-2 font-semibold border-l bg-muted/30">Invoiced</th>
              </tr>
            ...
            <tbody>
              {MONTHLY.map((r, i) => (
                <tr key={r.m} className="border-b last:border-0 hover:bg-muted/20">
                  <td className="p-2 font-medium">{i + 1}. {r.m}</td>
              ...
              <tr className="bg-accent/5 text-accent-foreground/80">
                ...
                <td colSpan={3} className="p-2 text-right border-l font-medium">{formatCurrency(annualB)}</td>
                <td className="p-2"></td>
                <td colSpan={3} className="p-2 text-right border-l font-medium">{formatCurrency(annualI)}</td>
                <td className="p-2"></td>
              </tr>
            </tbody>
          </table>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b">
                <th rowSpan={2} className="text-left p-2 font-medium text-muted-foreground align-bottom">Month</th>
                <th colSpan={3} className="text-center p-2 font-semibold border-l bg-muted/30">Lux</th>
                <th colSpan={3} className="text-center p-2 font-semibold border-l bg-muted/30">SW</th>
                <th colSpan={3} className="text-center p-2 font-semibold border-l bg-muted/30">FL</th>
              </tr>
              <tr className="border-b text-muted-foreground">
